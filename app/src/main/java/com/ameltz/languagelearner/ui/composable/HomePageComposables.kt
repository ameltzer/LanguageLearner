    ...
    if (decks.isEmpty()) {
        ...
        LazyColumn(
            ...
            items(decks) { deck ->
                ...
                var showViewDeckDialog by remember { mutableStateOf(false) }
                ...
                DropdownMenu(
                    ...
                    DropdownMenuItem(
                        text = { Text("View Study Deck") },
                        onClick = {
                            showMenu = false
                            showViewDeckDialog = true
                        },
                        leadingIcon = {
                ...
                if (showViewDeckDialog) {
                    StudyDeckViewDialog(
                        deckName = deck.deckName,
                        studyDeckId = deck.todaysDeckId,
                        homePageViewModel = homePageViewModel,
                        onDismiss = { showViewDeckDialog = false }
                    )
                }
            }
        }
...
@Composable
fun StudyDeckViewDialog(
    deckName: String,
    studyDeckId: Uuid,
    homePageViewModel: HomePageViewModel,
    onDismiss: () -> Unit
) {
    val cards = remember { homePageViewModel.getFullStudyDeckCards(studyDeckId) }

    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text("Study Deck: $deckName") },
        text = {
            Column(
                modifier = Modifier
                    .fillMaxWidth()
                    ...
                    cards.forEach { card ->
                        Column {
                            Row(
                                ...
                                Text(
                                    text = card.easyCount.toString(),
                                    style = MaterialTheme.typography.bodySmall,
                                    modifier = Modifier.weight(0.8f)
                                )
                                Text(
                                    text = card.mediumCount.toString(),
                                    style = MaterialTheme.typography.bodySmall,
                                    modifier = Modifier.weight(0.8f)
                                )
                                Text(
                                    text = card.hardCount.toString(),
                                    style = MaterialTheme.typography.bodySmall,
                                    modifier = Modifier.weight(0.8f)
                                )
                            }
